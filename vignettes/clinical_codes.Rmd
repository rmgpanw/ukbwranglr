...
If re-running the above search with `standardise_output` as `FALSE` and `preferred_description_only` as `TRUE`, note that some of the input code values may not appear in the result if multiple codes map to a single code:

```{r}
# Some Read2 codes for T1DM
t1dm_read2 <- c("C10E.", "C108.")

# lookup details - the input Read 2 code 'C10E.' does not appear in the result
map_codes(codes = t1dm_read2, 
          from = "read2", 
          to = "read3", 
          ukb_code_mappings = ukb_code_mappings, 
...
# attempting to map this to Read 2 returns an result however
map_codes(

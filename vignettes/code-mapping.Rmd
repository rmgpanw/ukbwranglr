...
Use `find_codes_by_description()`. For example, to find Read2 codes that match the description 'diabetic retinopathy':

```{r}
find_codes_by_description(reg_expr = "diabetic retinopathy", 
                          code_type = "read2", 
                          ukb_code_mappings = ukb_code_mappings, 
                          ignore_case = TRUE, 
                          codes_only = FALSE) %>% 
...
ukb_code_mappings$read_v2_icd10 %>% 
  filter(stringr::str_detect(icd10_code, pattern = "H360"))
```

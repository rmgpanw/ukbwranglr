...
3.  Summarise continuous variables with `summarise_continuous_variables()`.
4.  Tidy clinical events data and extract outcomes of interest with `tidy_clinical_events()` and `extract_phenotypes()`.
5.  Analyse.
...
2.  Create a clinical events database using `make_clinical_events_db()`. This function includes the option to incorporate [primary care data](https://biobank.ndph.ox.ac.uk/ukb/label.cgi?id=3000 "Primary care health-related outcomes") - it will take \~1 hour to run if including both the primary care clinical event records and prescription records datasets.

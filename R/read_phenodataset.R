...
#' @param delim Delimiter for the UKB phenotype file. Default is '\\t'
#' @param pheno_data_dict A data dictionary specific to the UKB phenotype file,
#'   generated by \code{\link{pheno_data_dict}}. This can be a filtered version
...
#
  ...
  pb <- progress::progress_bar$new(format = "[:bar] :current/:total (:percent)",
                                   total = length(unique(stats::na.omit(pheno_data_dict$Coding))))
  pb$tick(0)

  # loop through codings and parse columns depending on associated ValueType
  for (coding in unique(stats::na.omit(pheno_data_dict$Coding))) {
...
ukb_rename_columns <- function(pheno_data_dict) {
  ...
  column_names <- paste(pheno_data_dict$Field,
                        paste0("f", pheno_data_dict$FieldID),

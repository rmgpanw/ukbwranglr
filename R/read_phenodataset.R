...
#' @param ukb_data_dict The UKB data dictionary. This should be a data frame where all columns are of type \code{character}. By
#'   default, this is downloaded from the \href{https://github.com/rmgpanw/ukbwranglr_resources}{\code{ukbwranglr_resources}} github repo.
#'
...
make_data_dict <- function(path,
  ...
  # read ukb data dictionary from ukbwranglr_resources
  message("Getting data dictionary")
  ukb_data_dict <- get_ukb_data_dict()
  ...
  mapping_df <- mapping_df %>%
    dplyr::mutate(cont_int_to_na  = dplyr::case_when(
      ...
      # CONVERT TO NA
      Coding %in% ukbwranglr:::cont_and_int_codings_to_na ~ TRUE,

      # DO *NOT* CONVERT TO NA
      Coding %in% ukbwranglr:::cont_and_int_codings_NOT_to_na ~ FALSE,
      TRUE ~ FALSE
    ))
...
#'   generated by \code{\link{make_data_dict}}. This can be a filtered version
#'   of the full dictionary if only a subset of columns need to be loaded.
...
#'    website}.
#' @param ukb_codings The path to a tab-delimited UKB codings dictionary. By
#'   default, this is downloaded directly from the
#'   \href{https://biobank.ctsu.ox.ac.uk/crystal/exinfo.cgi?src=accessing_data_guide}{UKB
#'    website}.
...
#SETUP
  ...
  ukb_df <- data.table::fread(
    path,
    select = data_dict$field_id_full_raw, # selected cols
    colClasses = c('character'),
    na.strings = c("", "NA"),
    sep = delim,
    ...
  ...
  message("Downloading codings dictionary from UKB website")

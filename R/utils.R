...
#' @param scakar_output bool. If \code{TRUE}, error raised if more than one
...
extract_codings_for_fieldids <- function(field_ids,
  ...
  ukb_codings %>%
    dplyr::filter(Coding == (
      ukb_data_dict %>%
        dplyr::filter(FieldID %in% field_ids) %>%
        .$Coding %>%
        utils::head(n = 1)
    ))
}
...
download_dummy_ukb_data_to_tempdir <- function() {
  ...
  download.file("https://raw.githubusercontent.com/rmgpanw/ukbwranglr_resources/main/dummy_ukb_data/dummy_ukb_data.csv",
...
recode_column_coding_meaning_value_mapping_df <- function(field_id,
  ...
  if (mapping_direction == "meaning_code") {
    ukb_codings <- ukb_codings %>%
      dplyr::rename(old_vals = Meaning,
             new_vals = Value) %>%
      dplyr::select(-Coding)
  } else if (mapping_direction == "code_meaning") {
    ukb_codings <- ukb_codings %>%
      dplyr::rename(old_vals = Value,
             new_vals = Meaning) %>%
      dplyr::select(-Coding)
  }

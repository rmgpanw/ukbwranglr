...
#'
#' Reads a UK Biobank main dataset file into R. Optionally renames variables
#' with descriptive names and applies STATA-style variable and value labels
#' using the \code{\link[haven]{haven}} package.
#'
#' @param path The path to a UKB main dataset file.
#' @param delim Delimiter for the UKB main dataset file. Default is \code{\\t}
...
#'   \href{https://github.com/rmgpanw/ukbwranglr_resources}{\code{ukbwranglr_resources}}
#'    github repo.
#' @param n_labels_threshold Coded variables with associated value labels up to
#'   (and equal to) this threshold will be labelled using
...
read_ukb <- function(path,
  ...
  assert_integer_ge_1(n_labels_threshold,
                      "n_labels_threshold")
...
label_ukb_main <- function(ukb_main,
  ...
  if (!is.null(n_labels_threshold)) {
    ukb_codings <- ukb_codings %>%
      dplyr::group_by(.data[["Coding"]]) %>%
      dplyr::mutate("n" = dplyr::n()) %>%
      dplyr::ungroup() %>%
      dplyr::filter(.data[["n"]] <= n_labels_threshold)
  }
